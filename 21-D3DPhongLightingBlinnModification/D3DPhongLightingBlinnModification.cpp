...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("D3D11 Window : Phong Lighting Blinn Modification"),
		WS_OVERLAPPEDWINDOW,
		(x / 2) - (WIN_WIDTH / 2),
		(y / 2) - (WIN_HEIGHT / 2),
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	const char *pixelShaderSourceCode =
		...
		"float3 half_Vector = normalize(normalized_light_direction + normalized_viewer_vector);	\n" \
		"float4 specular = ls * ks * pow(max(dot(half_Vector, normalized_transformed_normals), 0.0), material_shininess);		\n" \
...
void update(void)
{
	gAngleSphere = gAngleSphere + 0.0005f;
	if (gAngleSphere > 360.0f)
	{
		gAngleSphere = 0.0f;
	}
}