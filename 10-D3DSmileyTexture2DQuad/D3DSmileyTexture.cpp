...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Smiley_Texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("D3D11 Window : Smiley Texture"),
		WS_OVERLAPPEDWINDOW,
		(x / 2) - (WIN_WIDTH / 2),
		(y / 2) - (WIN_HEIGHT / 2),
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Smiley_Texture);
	...
	ZeroMemory(&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Smiley_Texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
	memcpy(mappedSubResource.pData, smileyTexCoords, sizeof(smileyTexCoords));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Smiley_Texture, 0);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "LoadD3DTexture() Succeeded For Pyramid.\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(VDG_ATTRIBUTE_TEXTURE0, 1, &gpID3D11Buffer_VertexBuffer_Smiley_Texture, &stride, &offset);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Smiley_Texture)
	{
		gpID3D11Buffer_VertexBuffer_Smiley_Texture->Release();
		gpID3D11Buffer_VertexBuffer_Smiley_Texture = NULL;
	}
