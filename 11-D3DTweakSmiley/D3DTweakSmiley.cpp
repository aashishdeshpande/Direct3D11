...
//For Smiley
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Smiley_Position = NULL;
...
ID3D11SamplerState *gpID3D11SamplerState_Smiley_Texture = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("D3D11 Window : Tweak Smiley Texture"),
		WS_OVERLAPPEDWINDOW,
		(x / 2) - (WIN_WIDTH / 2),
		(y / 2) - (WIN_HEIGHT / 2),
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(smileyVertices);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Smiley_Position);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Smiley_Position, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
	memcpy(mappedSubResource.pData, smileyVertices, sizeof(smileyVertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Smiley_Position, 0);

	//create buffer for Pyramid Texture
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(smileyTexCoords);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	ZeroMemory(&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
	memcpy(mappedSubResource.pData, smileyTexCoords, sizeof(smileyTexCoords));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture, 0);
	...
	//For Smiley Texture
	hr = Load3DTexture(L"Smiley.bmp", &gpID3D11ShaderResourceView_Texture_Smiley);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "LoadD3DTexture() Failed For Smiley.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "LoadD3DTexture() Succeeded For Smiley.\n");
		fclose(gpFile);
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Smiley_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateSamplerState() Failed For Smiley Texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Smiley_Position, &stride, &offset);
	...
	case 1:
		...
		smileyTexoords[6] = 0.0f;
		smileyTexoords[7] = 0.5f;

		smileyTexoords[8] = 0.5f;
		smileyTexoords[9] = 0.0f;

		smileyTexoords[10] = 0.0f;
		smileyTexoords[11] = 0.0f;

		//copy Texture into the above buffer created for Texture
		ZeroMemory(&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
		memcpy(mappedSubResource.pData, smileyTexoords, sizeof(smileyTexoords));
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture, 0);


		stride = sizeof(float) * 2;
		offset = 0;
		gpID3D11DeviceContext->IASetVertexBuffers(VDG_ATTRIBUTE_TEXTURE0, 1, &gpID3D11Buffer_VertexBuffer_Texture, &stride, &offset);

		gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Texture_Smiley);
		gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Smiley_Texture);
	...
	case 4:
		...
		smileyTexoords[8] = 0.5f;
		smileyTexoords[9] = 0.5f;

		smileyTexoords[10] = 0.5f;
		smileyTexoords[11] = 0.5f;
	...
	case 0:
		ZeroMemory(&mappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
		memcpy(mappedSubResource.pData, smileyTexcoordsSquare, sizeof(smileyTexcoordsSquare));
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture, 0);
...
void uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_Smiley_Position)
	{
		gpID3D11Buffer_VertexBuffer_Smiley_Position->Release();
		gpID3D11Buffer_VertexBuffer_Smiley_Position = NULL;
	}
