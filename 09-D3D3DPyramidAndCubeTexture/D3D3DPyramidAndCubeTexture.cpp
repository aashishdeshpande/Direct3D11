...
HRESULT initialize(void)
{
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidTexCoords);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	memcpy(mappedSubResource.pData, pyramidTexCoords, sizeof(pyramidTexCoords));
	...
	//create buffer for Cube Texture
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(cubeTexCoords);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	memcpy(mappedSubResource.pData, cubeTexCoords, sizeof(cubeTexCoords));
	...
	hr = Load3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_Texture_Pyramid);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	//FOR CUBE
	hr = Load3DTexture(L"Kundali.bmp", &gpID3D11ShaderResourceView_Texture_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "LoadD3DTexture() Succeeded For Cube.\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(VDG_ATTRIBUTE_TEXTURE0, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Texture, &stride, &offset);
	...
	//for Texture dynamic draw of Square
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(VDG_ATTRIBUTE_TEXTURE0, 1, &gpID3D11Buffer_VertexBuffer_Cube_Texture, &stride, &offset);
