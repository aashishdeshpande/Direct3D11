...
float gAnglePerspectiveDeg = 450.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("D3D11 Window : MultiColored Triangle"),
		WS_OVERLAPPEDWINDOW,
		(x / 2) - (WIN_WIDTH / 2),
		(y / 2) - (WIN_HEIGHT / 2),
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Failed for vertices.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateInputLayout() Succeeded for vertices.\n");
		fclose(gpFile);
	}*/
...
void display(void)
{
	...
	//for color dynamic draw
	gpID3D11DeviceContext->IASetVertexBuffers(VDG_ATTRIBUTE_COLOR, 1, &gpID3D11Buffer_VertexBuffer_Color, &stride, &offset);
